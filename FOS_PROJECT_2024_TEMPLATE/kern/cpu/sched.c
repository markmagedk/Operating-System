...
//	panic("Not implemented yet");
    ...
    ProcessQueues.env_ready_queues = kmalloc(sizeof(struct Env_Queue)*numOfPriorities);    // Initialize each ready queue
	int SobeihCounter=0;
    while(SobeihCounter<numOfPriorities){
    	init_queue(&(ProcessQueues.env_ready_queues[SobeihCounter]));
    	SobeihCounter++;
    }
    quantums=kmalloc(sizeof(quantum));
...
//	panic("Not implemented yet");
	...
	struct Env *Sobeihcur_env = get_cpu_proc();
//	cprintf("cur_env is running");
	if(Sobeihcur_env!=NULL){
//		cprintf("element is running and not null");
	    sched_insert_ready(Sobeihcur_env);
	}

	struct Env *SobeihIterator=NULL;
	int SobeihCounter=0;
	while(SobeihCounter<num_of_ready_queues){
		if(!LIST_EMPTY(&(ProcessQueues.env_ready_queues[SobeihCounter]))){

						SobeihIterator=dequeue(&(ProcessQueues.env_ready_queues[SobeihCounter]));
		//				cprintf("element not equal null");
						SobeihIterator->env_status=ENV_UNKNOWN;
...
//
//				SobeihIterator=dequeue(&(ProcessQueues.env_ready_queues[i]));
////				cprintf("element not equal null");
//				SobeihIterator->env_status=ENV_UNKNOWN;
//				break;
//
...
//		panic("Not implemented yet");
			...
			acquire_spinlock(&(ProcessQueues.qlock));
			struct Env *SobeihElement=NULL;
			int SobeihCounter=1;
			while(SobeihCounter<num_of_ready_queues)
			{
				LIST_FOREACH(SobeihElement,&(ProcessQueues.env_ready_queues[SobeihCounter]))
		{
							if(SobeihElement!=NULL)
							{
							    if(SobeihElement->SobeihtrackNclock>threshold)
							    {
									env_set_priority(SobeihElement->env_id,SobeihElement->priority-1);
									SobeihElement->SobeihtrackNclock=0;
								}
							    else
							    {
										SobeihElement->SobeihtrackNclock++;
							    }
							}
...
//				LIST_FOREACH(SobeihElement,&(ProcessQueues.env_ready_queues[i])){
//					if(SobeihElement!=NULL){
//					if(SobeihElement->trackNclock>threshold){
//
//						env_set_priority(SobeihElement->env_id,SobeihElement->priority-1);
//						SobeihElement->trackNclock=0;
//					}
//					else{
//						SobeihElement->trackNclock++;
//					}
//				}
//
//			}
