...
void sys_wait_semaphore(struct Env_Queue *saad_queue,uint32 *lock){
	acquire_spinlock(&ProcessQueues.qlock);

	struct Env* saad_env = get_cpu_proc();
	*lock = 0;
	saad_env->env_status = ENV_BLOCKED;
	enqueue(saad_queue, saad_env);
...
void sys_signal_semaphore(struct Env_Queue *same7_queue){

	acquire_spinlock(&ProcessQueues.qlock);
	struct Env *same7_env=	dequeue(same7_queue);
	sched_insert_ready(same7_env);
	release_spinlock(&ProcessQueues.qlock);

}


void sys_init(struct Env_Queue *mustafa_saad_queue){

	init_queue(mustafa_saad_queue);
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	{
	    // Cast `a1` to a pointer to `struct semaphore` and dereference it
		struct Env_Queue *temp_semaphore = (struct Env_Queue *)a1;
	    sys_wait_semaphore(temp_semaphore,(uint32*)a2);
	    break;
	}
	case SYS_signal_semaphore:
	{
	    // Cast `a1` to a pointer to `struct semaphore` and dereference it
		struct Env_Queue *omar_semaphore = (struct Env_Queue *)a1;
	    sys_signal_semaphore(omar_semaphore);
	    break;
	}
	case SYS_init:
	{
	    // Cast `a1` to a pointer to `struct Env_Queue` and dereference it
	    struct Env_Queue *saad_queue = (struct Env_Queue *)a1;
	    sys_init(saad_queue);
	    break;
	}
