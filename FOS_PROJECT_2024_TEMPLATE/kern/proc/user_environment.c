...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
	...
	unsigned int kernStkSize =KERNEL_STACK_SIZE;
	struct Env *envi_ptr = (struct Env*)kmalloc(kernStkSize);
    if (envi_ptr == NULL)
    {
        panic("Failed to allocate memory for the user kernel stack!");
    }

    uint32 *page_directory = ptr_user_page_directory;
    uint32 *kernel_stack_address = (uint32*)(&(envi_ptr->kstack));
    unmap_frame(page_directory, (uint32)kernel_stack_address);

    return (void*)(uint32)envi_ptr;
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	e->block_user_hard_limit = daLimit;
	e->block_user_start = daStart;
	e->block_user_brk = daStart;
