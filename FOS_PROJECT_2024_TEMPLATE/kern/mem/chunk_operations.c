...
void* sys_sbrk(int numOfPages)
{
	...
	switch (numOfPages) {
	case 0:
		return (void*) env->block_user_brk;

	default:
		if (numOfPages > 0) {
			uint32 current_break = env->block_user_brk;
			uint32 new_break = env->block_user_brk + (numOfPages * PAGE_SIZE);

			if (new_break > env->block_user_hard_limit) {
				return (void*) -1;
			} else {
				env->block_user_brk = new_break;
				allocate_user_mem(env, (uint32) current_break,
...
//	return;
	...
	uint32 current_vaddr = virtual_address;
	int page_index = 0;
	while (page_index < size / PAGE_SIZE) {
		int table_status = get_page_table(e->env_page_directory,
				virtual_address + (page_index * PAGE_SIZE), &page_table_ptr);
		if (table_status == TABLE_NOT_EXIST) {
			create_page_table(e->env_page_directory,
					virtual_address + (page_index * PAGE_SIZE));
		}

		pt_set_page_permissions(e->env_page_directory,
				virtual_address + (page_index * PAGE_SIZE), PERM_MARKED, 0);
...
//	return;
	...
	uint32 current_addr = virtual_address;
	for (uint32 page_index = 0; page_index < size; page_index++) {
		pt_set_page_permissions(e->env_page_directory, current_addr, 0,
				PERM_MARKED);
		pf_remove_env_page(e, current_addr);

		env_page_ws_invalidate(e, current_addr);

		unmap_frame(e->env_page_directory, current_addr);
	...
	if (e->page_last_WS_element != NULL) {
		...
		{
			if (ws_element == e->page_last_WS_element) {
				break;
			} else {
				struct WorkingSetElement *temp_element = ws_element;
