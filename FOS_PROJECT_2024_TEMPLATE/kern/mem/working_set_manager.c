...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
	...
	//Your Code is Here...
	unsigned int viradd =virtual_address;
	unsigned int sizeOfMyWorkingSet = sizeof(struct WorkingSetElement);


	struct WorkingSetElement *itm = kmalloc(sizeOfMyWorkingSet );
	itm->virtual_address = viradd;
	return itm;
