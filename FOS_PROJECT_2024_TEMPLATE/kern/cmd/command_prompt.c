...
bool have_matched_pattern(char *str1, char *str2) {
...
int process_command(int number_of_arguments, char** arguments)
{
	...
	struct Command *remove_elment;
		LIST_FOREACH(remove_elment, &foundCommands)
		{
			LIST_REMOVE(&foundCommands , remove_elment);
		}
		int num_command_arg = number_of_arguments - 1;
		int i = 0;
		int command_find = 0;
		for (i = 0; i < NUM_OF_COMMANDS; i++)
		{
			int num_charcter_not_matched =  strcmp(arguments[0], commands[i].name);
			int current_command_leanth = strlen(commands[i].name);
			int num_current_command_arg = commands[i].num_of_args;
			if (num_charcter_not_matched == 0)
			{
				if(num_command_arg == num_current_command_arg||
						(num_command_arg >=1 && num_current_command_arg == -1))
				{
					return i;
				}
				else
			...
			else if(have_matched_pattern(commands[i].name , arguments[0]))
			{
				command_find=1;
				LIST_INSERT_TAIL(&foundCommands,&commands[i]);
			}
		...
		if(command_find==0) return CMD_INVALID;
		return CMD_MATCHED;
}
